...
class GameInvite(ConnectionMessageContainer):
    def __init__(self, general_conn_msg=None):
        if general_conn_msg is None:
            general_conn_msg = GeneralConnectionMessage(GAME_INVITE_ID)
        self.general_conn_msg = general_conn_msg

    def is_good_message_type(self):
        return self.general_conn_msg.message_type == GAME_INVITE_ID
